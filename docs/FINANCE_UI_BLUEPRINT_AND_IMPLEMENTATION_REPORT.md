...
### 3B. Screen Text & Actionability Checklist
Για κάθε screen section πρέπει να μπορείς να απαντήσεις ξεκάθαρα στα παρακάτω. Αν όχι, η οθόνη/section είναι πιθανόν “θολή” ή υπερβολικά φορτωμένη.
...
- **Date range rule**: Όλες οι βασικές οθόνες έχουν date range συμβατό με dashboard: current month, quarter, YTD, last-year-to-date, με σαφή ορισμό τι σημαίνει “period” ανά metric (βλ. section 10A).
...
This screen is monitoring-first, not execution-first.

**Entry from:** user navigation to `Finance Overview Dashboard` (landing/overview context).
...
- **KPI strip (primary monitoring pathways)**: 8–12 KPI cards (βλ. section 10A).
...
- Όλες οι κάρτες εμφανίζουν “definition tooltip” (σύντομος ορισμός) που παραπέμπει στο section 10A.
...
##### a. Screen Purpose
Να δίνει ορατότητα budgeted vs committed vs actual paid και variance, ως supporting control layer. This screen belongs to the supporting control layer. Δεν λειτουργεί ως execution workspace και δεν μετατρέπεται σε forecasting/GL.
...
##### a. Screen Purpose
Να προσφέρει ορατότητα κόστους προσωπικού για operational έλεγχο και margin insights, ως supporting control layer με role-based περιορισμούς. This screen belongs to the supporting control layer. Δεν ορίζει payroll/accounting.
...
### 10. Dashboard Metric Definitions (summary)
Αυτή η ενότητα είναι σύντομος index. Οι KPI/date semantics δίνονται στην canonical matrix του `### 10A`.
...
### 12A. UI-blocking decisions before build
...
3. **Partial payment allocation behavior**
   ...
   - Fallback: show “Unallocated amount” warning και outstanding ως total - sum(payments) μέχρι να οριστεί allocation granularity.
